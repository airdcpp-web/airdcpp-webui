...
const Row: React.FC<{ type: API.SearchType; moduleT: UI.ModuleTranslator; }> = ({ type, moduleT }) => (
  <tr>
    ...
    <td className="extensions">
      { type.extensions.join(', ') }
    </td>
    <td className="default">
      { formatBoolean(type.default_type, moduleT.plainT) }
    </td>
  </tr>
);
...
const SearchTypesPage: React.FC<Props> = (
  { searchTypes, moduleT }
) => (
  <div id="directory-table">
    <ActionButton
      actions={ SearchTypeActions.create }
      actionId="create"
    />
    ...
    { searchTypes.length === 0 ? null : (
      <table className="ui striped table">
        ...
        <tbody>
          { searchTypes.map(type => getRow(type, moduleT)) }
        </tbody>
      </table>
    ) }
    <Message
...
export default DataProviderDecorator<SearchTypesPageProps, DataProps>(
  SearchTypesPage, {
    urls: {
