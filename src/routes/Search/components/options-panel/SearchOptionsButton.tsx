...
const removeEmptyProperties = (value: SearchOptions) => {
  const ret = Object.keys(value).reduce((reduced, key) => {
...
const SearchOptionsButtonDecorated = DataProviderDecorator<
...
>(SearchOptionsButton, {
  ...
  dataConverters: {
    hubs: (hubs: API.Hub[]) =>
      hubs.filter((hub) => hub.connect_state.id === API.HubConnectStateEnum.CONNECTED),
  },
});
