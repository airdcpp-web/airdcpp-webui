...
const SearchTypeDropdown: React.FC<Props> = ({ searchTypes, value, onChange, moduleT }) => {
  const commonItems = getCommonItems(moduleT);
  const selectedItem = searchTypes.find(i => value === i.id) || commonItems.find(i => value === i.id) || commonItems[0];
  return (
    <Dropdown
      selection={ true }
      caption={ (
